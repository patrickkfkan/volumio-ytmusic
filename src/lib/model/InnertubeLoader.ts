...
export default class InnertubeLoader {
  ...
  static async #beginPOStage(innertube: Innertube, auth: Auth, resolve: (value: InnertubeLoaderGetInstanceResult) => void, lastToken?: POToken) {
    ...
    if (lastIdentifier) {
      ...
      if (!identifier) {
        ytmusic.getLogger().warn('[ytmusic] InnertubeLoader: signed in but could not get datasyncIdToken for fetching po_token');
      }
    }
    else {
      ...
      case Stage.Init:
        if (event === AuthEvent.SignIn || event === AuthEvent.Pending) {
          this.#beginPOStage(innertube, auth, resolve)
            .catch((error: unknown) => {
              ytmusic.getLogger().error(ytmusic.getErrorMessage(`[ytmusic] InnertubeLoader: error creating Innertube instance (with po_token):`, error));
            });
          return;
        }
        ytmusic.getLogger().warn('[ytmusic] InnertubeLoader: po_token was not used to create Innertube instance because of auth error or unknown auth status. Playback of YouTube content might fail.');
        this.#resolveGetInstanceResult(innertube, auth, resolve);
        return;
      case Stage.PO:
